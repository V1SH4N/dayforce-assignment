...
{
    public class AidaChatService
    {
        private readonly Kernel _kernel;
        private readonly IHttpClientFactory _httpClientFactory;

        public AidaChatService(Kernel kernel, IHttpClientFactory httpClientFactory)
        {
            _kernel = kernel;
        ...
        public async Task<string> GetEconomiesOfScaleAdvantageAsync(CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                byte[] imgBytes = await client.GetByteArrayAsync("rest/api/3/attachment/content/568297");

                // ensure bytes not empty
                if (imgBytes == null || imgBytes.Length == 0) return "Downloaded image is empty.";
                var history = new ChatHistory();
                history.AddUserMessage("Explain the image");
                history.AddUserMessage([new ImageContent(new ReadOnlyMemory<byte>(imgBytes), "image/jpeg")]);
                //    new ChatMessageContentItemCollection
                //{
                //    new TextContent("Explain this image:"),
                //    //new ImageContent("image/jpeg", img)
                //    new ImageContent(new ReadOnlyMemory<byte>(imgBytes), "image/jpeg")
                //});

                //new ChatResponseFormat
                //new 

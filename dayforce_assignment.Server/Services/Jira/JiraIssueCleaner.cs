...
public class JiraIssueCleaner : IJiraIssueCleaner
{
    ...
    public JiraIssueDto CleanJiraIssue(JsonElement jiraJson, JsonElement jiraRemoteLinks)
    {

        // Jira id
        var jiraKey = jiraJson.TryGetProperty("key", out var keyProp) ? keyProp.GetString() : string.Empty;

        var dto = new JiraIssueDto { Key = jiraKey};

        // Jira title
        if (jiraJson.TryGetProperty("fields", out var fields))
            dto.Title = fields.TryGetProperty("summary", out var summary) ? summary.GetString() : string.Empty;
       

        // Parent Issue
        if (fields.TryGetProperty("parent", out var parent))
            dto.ParentKey = parent.TryGetProperty("key", out var parentKey) ? parentKey.GetString() : string.Empty;
        ...
        if (fields.TryGetProperty("project", out var project))
        {
            ...
            {
                Key = project.TryGetProperty("key", out var projKey) ? projKey.GetString() : string.Empty,
                Name = project.TryGetProperty("name", out var projName) ? projName.GetString() : string.Empty
            };
        }
        ...
        if (fields.TryGetProperty("subtasks", out var subtasks) && subtasks.ValueKind == JsonValueKind.Array)
        {
            ...
            {
                var subKey = st.TryGetProperty("key", out var k) ? k.GetString() : string.Empty;
                var summaryText =
                    st.TryGetProperty("fields", out var f) && f.TryGetProperty("summary", out var s) ? s.GetString() : string.Empty;
            ...
            attachments.ValueKind == JsonValueKind.Array)
        {
            ...
            {
                var content = attachment.TryGetProperty("content", out var c) ? c.GetString() : string.Empty;
                var mimeType = attachment.TryGetProperty("mimeType", out var t) ? t.GetString() : string.Empty;
